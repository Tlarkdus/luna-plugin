...
| 종류 | 이름 | 상태 |
|---|---|---|
| 커맨드 | `/repo-grade` — 저장소를 AI-Ready 관점에서 100점 채점 | 사용 가능 |
| 스킬 | `repo-grade` — 채점 루브릭 (커맨드가 읽는다) | 사용 가능 |
| 훅 | `PreToolUse` — Edit/Write는 TDD 가드, Bash는 위험 명령 차단 | 사용 가능 |
| 스크립트 | `tdd-guard.sh` — 훅이 부른다 | 사용 가능 |
